...
{
	float z;
	z = ((-1-(x*x*x*x*y*y))/(x*x*x));  //calculates value of function and stores it in variable z.
...
rk2o()
{
	...
	*/
    ...
    for(i=1;i<=count;i++)
    {
        ...
        k2=h*equation(xvalue[i]+h,yvalue[i]+k1);
...
*/
	...
	*/
    ...
    for(i=1;i<=count;i++)
    {
        k1 = (h/3)*equation(xvalue[i],yvalue[i]); //value of k1

        k2 = (h/3)*equation(xvalue[i]+(h/2),yvalue[i]+(k1/2));

        k3 = (h/3)*equation(xvalue[i]+(h/2),yvalue[i]+(k2/2));

        k4 = (h/3)*equation(xvalue[i]+h,yvalue[i]+k3);

        k = (k1+(2*k2)+(2*k3)+k4)/6 ;//calculates value of k
